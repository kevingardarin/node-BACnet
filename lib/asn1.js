...
// encodage pour ecriture exception_schedule (date, date_range, week&day, reference)
const encodeContextSpecificDecodedExceptionSchedule = module.exports.encodeContextSpecificDecodedExceptionSchedule = (buffer, value) => {
  // boucle sur toutes les exceptions saisies
  value.forEach(exception => {
        ...
        let priority = exception.priority;
            ...
            if (type == 114) {
              ...
              encodeBacnetExceptionDate(buffer, value)
              encodeClosingTag(buffer, 0);
            } else if (type == 102) {
              encodeOpeningTag(buffer, 0);
              encodeOpeningTag(buffer, 1);
              encodeBacnetExceptionDateRange(buffer, value)
              encodeClosingTag(buffer, 1);
              encodeClosingTag(buffer, 0);
            } else if (type == 101) {
              encodeOpeningTag(buffer, 0);
              encodeBacnetExceptionWeekDay(buffer, value)
              encodeClosingTag(buffer, 0);
            } else if (type == 75) {
              encodeBacnetExceptionReference(buffer, value)
            }

            times.forEach((clock, index) => {
                if (index == 0) {
                  encodeOpeningTag(buffer, 2);
                }
                if (clock.value != null) {
                    if (clock.type == 1) {
                      encodeApplicationTime(buffer, clock.time);
                      encodeApplicationBoolean(buffer, clock.value);
                    } else if (clock.type == 9) {
                      encodeApplicationTime(buffer, clock.time);
                      encodeApplicationEnumerated(buffer, clock.value);
                    } else if (clock.type == 4) {
                      encodeApplicationTime(buffer, clock.time);
                      encodeApplicationReal(buffer, clock.value);
                    }
                } else {
                  encodeApplicationTime(buffer, clock.time);
                ...
                if (index == times.length - 1) {
                  encodeClosingTag(buffer, 2)
                }
            })

   // a mettre a la fin de chaque exception (ex : event priority (Unsigned) 16)
   buffer.buffer[buffer.offset++] = 57;
   buffer.buffer[buffer.offset++] = priority;
  })
};


// encodage pour ecriture date list calendar (date, date_range, week&day)
const encodeContextSpecificDecodedDateList = module.exports.encodeContextSpecificDecodedDateList = (buffer, value) => {
  value.forEach(dates => {
        let value = dates.value;
        let type = dates.type;
        if (type == 114) {
           encodeBacnetExceptionDate(buffer, value)
        } else if (type == 102) {
...
// encodage pour ecriture effective period schedule
const encodeContextSpecificDecodedEffectivePeriod = module.exports.encodeContextSpecificDecodedEffectivePeriod = (buffer, value) => {
  encodeBacnetExceptionDateRange(buffer, value[0].value)
};

const encodeBacnetExceptionDate = (buffer, exception) => {
  buffer.buffer[buffer.offset++] = 12;
  buffer.buffer[buffer.offset++] = (exception.year == 255) ? 255 : exception.year - 1900;
  buffer.buffer[buffer.offset++] = (exception.month == 255) ? 255 : exception.month;
  buffer.buffer[buffer.offset++] = (exception.day == 255) ? 255 : exception.day;
  buffer.buffer[buffer.offset++] = (exception.wday == 255) ? 255 : exception.wday;
};

const encodeBacnetExceptionDateRange = (buffer, exception) => {
  let startDate = exception.startDate
  let endDate = exception.endDate
  const any = new Date (1901, 1, 1)
  // encodage date de début
  buffer.buffer[buffer.offset++] = 164;
  if (any - startDate == 0) {
      buffer.buffer[buffer.offset++] = 255;
      buffer.buffer[buffer.offset++] = 255;
      buffer.buffer[buffer.offset++] = 255;
      buffer.buffer[buffer.offset++] = 255;
  } else {
      buffer.buffer[buffer.offset++] = startDate.getFullYear() - 1900;
      buffer.buffer[buffer.offset++] = startDate.getMonth();
      buffer.buffer[buffer.offset++] = startDate.getDate();
      buffer.buffer[buffer.offset++] = startDate.getDay();
  }
  // encodage date de fin
  buffer.buffer[buffer.offset++] = 164;
  if (any - endDate == 0) {
      buffer.buffer[buffer.offset++] = 255;
      buffer.buffer[buffer.offset++] = 255;
      ...
      buffer.buffer[buffer.offset++] = endDate.getFullYear() - 1900;
      buffer.buffer[buffer.offset++] = endDate.getMonth();
      buffer.buffer[buffer.offset++] = endDate.getDate();
      buffer.buffer[buffer.offset++] = endDate.getDay();
  }
};

const encodeBacnetExceptionWeekDay = (buffer, exception) => {
  buffer.buffer[buffer.offset++] = 43;
  buffer.buffer[buffer.offset++] = exception.month;
  buffer.buffer[buffer.offset++] = exception.week;
  buffer.buffer[buffer.offset++] = exception.wday;
};

const encodeBacnetExceptionReference = (buffer, reference) => {
  buffer.buffer[buffer.offset++] = 28;
  buffer.buffer[buffer.offset++] = 1;
  buffer.buffer[buffer.offset++] = 128;
  buffer.buffer[buffer.offset++] = 0;
  buffer.buffer[buffer.offset++] = reference.instance;
};

// encodage pour ecriture weekly_schedule (bool, real, enum)
const encodeContextSpecificDecodedWeeklySchedule = module.exports.encodeContextSpecificDecodedWeeklySchedule = (buffer, value) => {
  // boucle sur tous les jours de la semaine
  value.forEach(daySchedule => {
    encodeOpeningTag(buffer, 0);
    // boucle toutes les plages horaires d'une journée
    daySchedule.forEach(schedule => {
      encodeApplicationTime(buffer, schedule.time);
        //encodage en fonctiopn du type de programme horaire (1=bool; 2=enum; 3=real)
        if (schedule.value != null) {
            if (schedule.type == 1) {
              encodeApplicationBoolean(buffer, schedule.value);
            } else if (schedule.type == 2) {
              encodeApplicationEnumerated(buffer, schedule.value);
            } else {
              encodeApplicationReal(buffer, schedule.value);
            }
        } else {
            encodeApplicationNull(buffer);
        }
...
const bacappEncodeApplicationData = module.exports.bacappEncodeApplicationData = (buffer, value) => {
  ...
  switch (value.type) {
    ...
    case baEnum.ApplicationTags.WEEKLY_SCHEDULE:
      encodeContextSpecificDecodedWeeklySchedule(buffer, value.value);
      break;
    case baEnum.ApplicationTags.SPECIAL_EVENT:
      encodeContextSpecificDecodedExceptionSchedule(buffer, value.value);
      break;
    case baEnum.ApplicationTags.DATE_LIST:
      encodeContextSpecificDecodedDateList(buffer, value.value);
      break;
    case baEnum.ApplicationTags.EFFECTIVE_PERIOD:
      encodeContextSpecificDecodedEffectivePeriod(buffer, value.value);
      break;
    default:
...
const decodeCalendarDate = (buffer, offset) => {
  ...
  const year = buffer[offset] === 255 ? buffer[offset] : buffer[offset] + yearOffset;
  return {
    len: 4,
    year,
...
const bacappDecodeContextApplicationData = (buffer, offset, maxOffset, objectType, propertyId) => {
  ...
  if (isContextSpecific(buffer[offset])) {
    ...
    // decodage spécifique pour reponse lors d'une lecture de ExceptionSchedule
    if (tagResult.tagNumber == 0 && tagResult.value == 4) {
        result = decodeCalendarDate(buffer, offset + tagResult.len);
        result = {
           len: result.len,
           type: 114,
           value: {year: result.year, month: result.month, day: result.day, wday: result.wday}
        }
        len += result.len;
        return result
    } else if (tagResult.tagNumber == 2 && tagResult.value == 3) {
        result = decodeCalendarWeekDay(buffer, offset + tagResult.len);
        result = {
           len: result.len,
           type: 101,
           value: {month: result.month, week: result.week, wday: result.wday}
        }
        len += result.len;
        return result
    } else if (tagResult.tagNumber == 1 && tagResult.value == 4) {
        result = decodeObjectId(buffer, offset + tagResult.len);
        result = {
           len: result.len,
           type: 75,
           value: [{len: result.len, type: 75, value: {type: result.objectType, instance: result.instance}}]
        }
        len += result.len;
        return result
    } else if (tagResult.tagNumber == 3 && tagResult.value == 1) {
        result = {
           len: 2,
           type: 57,
           value: {type: 57, priority: buffer[offset + tagResult.len]}
        }
        len += result.len;
        return result
    }
    ...
    while (((len + offset) <= maxOffset) && !isClosingTag(buffer[offset + len])) {
      ...
      if (!subResult) return;
        result = bacappDecodeApplicationData(buffer, offset + len + subResult.len, maxOffset, baEnum.ASN1_MAX_OBJECT_TYPE, baEnum.ASN1_MAX_PROPERTY_ID);
        len += result.len;
        if (!result) return;
          list.push(result);
