...
module.exports.encode = (buffer, funct, destination, source, hopCount, networkMsgType, vendorId, network, sadr) => {
  ...
  const octet1 = (network >> 8) & 0xFF;
  const octet2 = network & 0xFF;
  ...
  if (network != -1 && sadr != -1) {
    buffer.buffer[buffer.offset++] = '0x' + octet1.toString(16).padStart(2, '0');
    buffer.buffer[buffer.offset++] = '0x' + octet2.toString(16).padStart(2, '0');
    buffer.buffer[buffer.offset++] = 0x01
    buffer.buffer[buffer.offset++] = '0x' + sadr.toString(16).padStart(2, '0');
