...
module.exports.encode = (buffer, funct, destination, source, hopCount, networkMsgType, vendorId, network, sadr) => {
  network = network || -1;
  sadr = sadr || -1;
  const hasDestination = destination && destination.net > 0;
  const hasSource = source && source.net > 0 && source.net !== 0xFFFF;
  const octet1 = (parseInt(network) >> 8) & 0xFF;
  const octet2 = parseInt(network) & 0xFF;

  buffer.buffer[buffer.offset++] = BACNET_PROTOCOL_VERSION;
  buffer.buffer[buffer.offset++] = funct | baEnum.NpduControlBits.DESTINATION_SPECIFIED | (hasSource ? baEnum.NpduControlBits.SOURCE_SPECIFIED : 0);
  if (network != -1 && sadr != -1) {
    buffer.buffer[buffer.offset++] = '0x' + parseInt(octet1).toString(16);
    buffer.buffer[buffer.offset++] = '0x' + parseInt(octet2).toString(16);
    buffer.buffer[buffer.offset++] = 0x01
    buffer.buffer[buffer.offset++] = '0x' + parseInt(sadr).toString(16);
